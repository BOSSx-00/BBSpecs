...
`.github/workflows/release.yml` runs on a tag matching `v*`. It builds both platforms,
compiles the installer, and publishes a GitHub release with every file attached.
...
The tag follows `<Version>` in the `.csproj`, which stays the one place a version
number lives.

## Signing

Releases are currently unsigned, so Windows shows **Publisher: Unknown** on the
...
open-source projects for free and issues the certificate to itself, so the publisher
reads "SignPath Foundation". Signing runs in CI rather than locally, which is the
point: the certificate never touches a developer machine, and SignPath verifies the
artifact came from this workflow on this repository. The step is in the release
workflow, commented out, waiting on approval.

One thing to fix when enabling it: the step has to run *after* the upload it
references, and the signed result needs uploading again so the release job publishes
that rather than the original.

If you hold a certificate yourself, the build signs locally too:
...
machine that does not already trust it, which is every machine but yours.
