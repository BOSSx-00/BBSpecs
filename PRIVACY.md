...
telemetry, no analytics, and nothing is ever sent anywhere for us to look at.

BBSpecs makes four kinds of outbound connection, all listed below. None of them
carry anything about your hardware, and three of the four you can avoid
entirely.

## What stays on your computer

Everything BBSpecs reads about your machine is read, displayed and then
forgotten. It is never uploaded, never stored on a server, and never seen by
anyone but you. That includes:
...
This is the most frequent thing it does on the network and it is entirely
automatic, so it is listed first.
...
offline, and everything else continues to work.
...
If you choose to install an update, the new file is downloaded from GitHub in
the same way.

To avoid this entirely, block BBSpecs in your firewall. Every reading about
your own hardware works with no internet connection at all.
...
you, along with the rough location and provider name associated with it. This
is the address the rest of the internet already sees whenever you visit any
website.
...
disconnect a VPN. That is so it cannot sit there showing an address that is no
longer yours. This only happens after you have asked at least once, and it stops
...
on screen and kept only in memory. It is not written to disk and is not sent
anywhere else.
...
- It has no analytics, telemetry, usage tracking or crash reporting
- It has no account, no sign-in and no licence check
- It never uploads your hardware details, serial numbers or addresses
- It never sells or shares anything, because it never collects anything to share
- It does not read your documents, browsing history or any file except the
...
The crash log is never sent anywhere. If BBSpecs fails, it tells you where the
file is so you can look at it or attach it to a bug report yourself, if you
want to.

Uninstalling BBSpecs deliberately leaves your settings in place, so that
...
paste it when asking for help. It deliberately leaves out serial numbers and
...
BBSpecs asks for Administrator when it starts. This is because Windows only
allows elevated programs to read temperature sensors, fan speeds and drive
health. It is used for reading those sensors and nothing else.

Processor temperature additionally needs a kernel driver, which Windows offers
no way around. BBSpecs carries the official [PawnIO](https://pawnio.eu)
installer and offers to run it the first time it is needed. It is never
installed without asking, it runs entirely on your machine, and everything
except processor and motherboard temperature works without it.
