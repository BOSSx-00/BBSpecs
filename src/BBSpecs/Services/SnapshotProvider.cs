...
public abstract class SnapshotProvider : IDisposable
{
    ...
    // Enumerating drivers takes seconds, so it happens off to one side and the
    // page shows whatever has arrived. Holding the first snapshot back for it
    // would mean staring at the loading screen for no good reason.
    private List<DriverInfo> _drivers = [];
    private volatile bool _driversReady;
    private Task? _driverSweep;
    private DateTime _driverStamp = DateTime.MinValue;

    /// <summary>Drivers do not change while you look at them.</summary>
    private static readonly TimeSpan DriverLifetime = TimeSpan.FromMinutes(30);
    ...
    /// <summary>
    /// Kicks off a driver sweep if one is due and none is already running.
    /// Deliberately fire and forget: the result lands in the cache and appears
    /// in whichever snapshot happens to be built next.
    /// </summary>
    private void StartDriverSweep()
    {
        if (_driverSweep is { IsCompleted: false }) return;
        if (DateTime.UtcNow - _driverStamp < DriverLifetime) return;

        _driverStamp = DateTime.UtcNow;
        _driverSweep = Task.Run(() =>
        {
            List<DriverInfo> found = Safe(ReadDrivers, _drivers);
            if (found.Count > 0) _drivers = found;
            _driversReady = true;
        });
    }
    ...
    /// <summary>
    /// Installed drivers and their dates. Called on a background thread, never
    /// on the collector thread, because it is slow.
    /// </summary>
    protected virtual List<DriverInfo> ReadDrivers() => [];
