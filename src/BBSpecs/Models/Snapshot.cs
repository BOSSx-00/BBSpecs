...
public sealed class Snapshot
{
    ...
    /// <summary>
    /// Device drivers with their ages. Collected in the background because
    /// enumerating them takes seconds, so this is empty on the first few ticks.
    /// </summary>
    public List<DriverInfo> Drivers { get; set; } = [];

    /// <summary>False until the first driver sweep has finished.</summary>
    public bool DriversReady { get; set; }
...
/// <summary>
/// One installed driver.
///
/// Note what is deliberately missing: whether a newer one exists. Windows has
/// no service that answers that, and the vendors' own answers live behind
/// detection tools and undocumented endpoints that change without warning.
/// Rather than guess, BBSpecs reports how old the installed driver is, which is
/// a fact, and lets that speak for itself.
/// </summary>
public sealed class DriverInfo
{
    public string Device { get; set; } = "";

    /// <summary>Graphics | Audio | Network | Chipset | System firmware</summary>
    public string Category { get; set; } = "";
    public string? Version { get; set; }

    /// <summary>The driver's release date, already formatted for display.</summary>
    public string? Date { get; set; }
    public int? AgeDays { get; set; }
    public string? Vendor { get; set; }

    /// <summary>Where this vendor publishes drivers, when there is a sensible page.</summary>
...
public sealed class DriveInfo
{
    ...
    /// <summary>
    /// How much of the disk is actually carved into partitions, hidden ones
    /// included. Not the same as the volumes below: a Windows disk carries an
    /// EFI, an MSR and a recovery partition that have no drive letter and so
    /// never appear there. Comparing the disk size against the volumes made a
    /// perfectly normal drive look like it had tens of gigabytes going spare.
    /// Null when the partition table could not be read.
    /// </summary>
    public double? PartitionedGb { get; set; }
