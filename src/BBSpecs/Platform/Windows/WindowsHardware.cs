...
public sealed partial class WindowsHardware : SnapshotProvider
{
    ...
    protected override List<DriveInfo> ReadDrives()
    {
        ...
        Dictionary<int, List<char>> partitions = ReadPartitionMap();
            ...
            "FirmwareRevision FROM Win32_DiskDrive"))
        {
            ...
            drive.Volumes = partitions.TryGetValue(index, out List<char>? letters)
    ...
    /// </summary>
    private static Dictionary<int, List<char>> ReadPartitionMap()
    {
        var map = new Dictionary<int, List<char>>();

        foreach (ManagementObject part in Wmi.Query(
            "SELECT DiskNumber, DriveLetter FROM MSFT_Partition", @"root\Microsoft\Windows\Storage"))
        {
            char? letter = PartitionLetter(part);
            if (letter is null) continue;

            int disk = (int)part.Num("DiskNumber");
            if (!map.TryGetValue(disk, out List<char>? letters)) map[disk] = letters = [];
